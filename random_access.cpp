...
int main(int argc, char* argv[]) 
    ...
    long int NUM_OF_ACCESS = atoi( argv[2] );
    ...
    for(i = 0; i < DATA_SIZE; i++) 
        {
        a.push_back( rand() / DATA_SIZE );
        // b.push_back( (float) rand() / RAND_MAX );
        }

    cl_mem cl_a = clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR, sizeof(cl_long) * DATA_SIZE, &a[0], NULL);
    //cl_mem cl_b = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(cl_float) * DATA_SIZE, &b[0], NULL);
    //cl_mem cl_res = clCreateBuffer(context, CL_MEM_WRITE_ONLY, sizeof(cl_float) * DATA_SIZE, NULL, NULL);

    cl_long d_data_size = NUM_OF_ACCESS;
    ...
    */
    cl_program program = load_program(context, "random_access_kernel.cl");
    if(program == 0) 
        {
    ...
    cl_kernel random_access = clCreateKernel(program, "random_access", 0);
    if(random_access == 0) 
        {
    ...
    clSetKernelArg(random_access, 0, sizeof(cl_mem), &cl_a);
    //clSetKernelArg(random_access, 1, sizeof(cl_mem), &cl_b);
    //clSetKernelArg(random_access, 2, sizeof(cl_mem), &cl_res);
    clSetKernelArg(random_access, 1, sizeof(cl_mem), &cl_data_size);

    size_t work_size = 1;
    cl_int err;
    cl_event event;
    clock_gettime(CLOCK_REALTIME, &kernel_start_time);
    err = clEnqueueNDRangeKernel(queue, random_access, 1, 0, &work_size, 0, 0, 0, &event);
    clFinish(queue);
